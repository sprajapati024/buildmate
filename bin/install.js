 ...
 * Buildmate Installer
 *
 * NOTE: This file contains legacy variable names referencing "GSD" (the original
 * project name). This is internal technical debt with no user-facing impact.
 * New contributions should use "Buildmate" naming. Legacy naming will be
 * gradually refactored over time.
 */
...
const GSD_CODEX_MARKER = '# Buildmate Agent Configuration \u2014 managed by buildmate-core installer';

// Copilot instructions marker constants
const GSD_COPILOT_INSTRUCTIONS_MARKER = '<!-- Buildmate Configuration \u2014 managed by buildmate-core installer -->';
const GSD_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /Buildmate Configuration -->';
...
Then re-run: npx buildmate-ai@latest
`);
    process.exit(1);
  }
}
...
const banner = '\n' +
  ...
  '  ╚═╝└─┘┴─┘ ┴ └─┘┴ ┴ ┴ └─┘' + reset + '\n' +
  '\n' +
  '  Buildmate ' + dim + 'v' + pkg.version + reset + '\n' +
  '  Multi-agent project development system for Claude Code\n';
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx buildmate-ai [options]\n\n  ${yellow}Options:${reset}\n    ${cyan}-g, --global${reset}              Install globally (to config directory)\n    ${cyan}-l, --local${reset}               Install locally (to current directory)\n    ${cyan}--claude${reset}                  Install for Claude Code only\n    ${cyan}--opencode${reset}                Install for OpenCode only\n    ${cyan}--gemini${reset}                  Install for Gemini only\n    ${cyan}--codex${reset}                   Install for Codex only\n    ${cyan}--copilot${reset}                 Install for Copilot only\n    ${cyan}--antigravity${reset}             Install for Antigravity only\n    ${cyan}--all${reset}                     Install for all runtimes\n    ${cyan}-u, --uninstall${reset}           Uninstall Buildmate (remove all Buildmate files)\n    ${cyan}-c, --config-dir <path>${reset}   Specify custom config directory\n    ${cyan}-h, --help${reset}                Show this help message\n    ${cyan}--force-statusline${reset}        Replace existing statusline config\n\n  ${yellow}Examples:${reset}\n    ${dim}# Interactive install (prompts for runtime and location)${reset}\n    npx buildmate-ai\n\n    ${dim}# Install for Claude Code globally${reset}\n    npx buildmate-ai --claude --global\n\n    ${dim}# Install for Gemini globally${reset}\n    npx buildmate-ai --gemini --global\n\n    ${dim}# Install for Codex globally${reset}\n    npx buildmate-ai --codex --global\n\n    ${dim}# Install for Copilot globally${reset}\n    npx buildmate-ai --copilot --global\n\n    ${dim}# Install for Copilot locally${reset}\n    npx buildmate-ai --copilot --local\n\n    ${dim}# Install for Antigravity globally${reset}\n    npx buildmate-ai --antigravity --global\n\n    ${dim}# Install for Antigravity locally${reset}\n    npx buildmate-ai --antigravity --local\n\n    ${dim}# Install for all runtimes globally${reset}\n    npx buildmate-ai --all --global\n\n    ${dim}# Install to custom config directory${reset}\n    npx buildmate-ai --codex --global --config-dir ~/.codex-work\n\n    ${dim}# Install to current project only${reset}\n    npx buildmate-ai --claude --local\n\n    ${dim}# Uninstall Buildmate from Codex globally${reset}\n    npx buildmate-ai --codex --global --uninstall\n\n  ${yellow}Notes:${reset}\n    The --config-dir option is useful when you have multiple configurations.\n    It takes priority over CLAUDE_CONFIG_DIR / GEMINI_CONFIG_DIR / CODEX_HOME / COPILOT_CONFIG_DIR / ANTIGRAVITY_CONFIG_DIR environment variables.\n`);
  process.exit(0);
}
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all buildmate: references → gsd-)
  c = c.replace(/buildmate:/g, 'gsd-');
  return c;
}
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all buildmate: references → gsd-)
  c = c.replace(/buildmate:/g, 'gsd-');
  return c;
}
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/buildmate:([a-z0-9-]+)/gi, (_, commandName) => {
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const codexGsdPath = `${path.resolve(targetDir, 'buildmate-core').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/buildmate-core prefix so path resolves to codex GSD install
    content = content.replace(/~\/\.claude\/buildmate-core\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/buildmate-core\//g, codexGsdPath);
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /buildmate:command with /gsd-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/buildmate:/g, '/gsd-');
...
/**
 * Uninstall Buildmate from the specified directory for a specific runtime
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling Buildmate from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 2. Remove buildmate-core directory
  const gsdDir = path.join(targetDir, 'buildmate-core');
  if (fs.existsSync(gsdDir)) {
    fs.rmSync(gsdDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed buildmate-core/`);
  }
          ...
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              ...
              for (const key of keys) {
                if (key.includes('buildmate-core')) {
  ...
  console.log(`
  ${green}Done!${reset} Buildmate has been uninstalled from ${runtimeLabel}.
 ...
 * This prevents permission prompts when GSD accesses the buildmate-core directory
...
function configureOpencodePermissions(isGlobal = true) {
  ...
  const gsdPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/buildmate-core/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/buildmate-core/*`;
...
function writeManifest(configDir, runtime = 'claude') {
  ...
  const isCopilot = runtime === 'copilot';
  const isAntigravity = runtime === 'antigravity';
  const gsdDir = path.join(configDir, 'buildmate-core');
  ...
  for (const [rel, hash] of Object.entries(gsdHashes)) {
    manifest.files['buildmate-core/' + rel] = hash;
  }
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>buildmate-core/bin/...
  ...
  // Display Buildmate logo
  const logoPath = path.join(src, 'assets', 'buildmate-logo.txt');
  ...
  if (isOpencode) {
    ...
    // V2 HARD CUT: GSD commands removed
    // Remove old GSD commands if they exist
    const gsdDest = path.join(commandsDir, 'gsd');
    if (fs.existsSync(gsdDest)) {
      fs.rmSync(gsdDest, { recursive: true, force: true });
      console.log(`  ${yellow}⚠${reset} Removed GSD commands (V2 hard cut)`);
    }
    
    // Copy buildmate commands (V2 - pure Buildmate, no GSD)
    const buildmateSrc = path.join(src, 'commands', 'buildmate');
    if (fs.existsSync(buildmateSrc)) {
      const buildmateDest = path.join(commandsDir, 'buildmate');
      copyWithPathReplacement(buildmateSrc, buildmateDest, pathPrefix, runtime, true, isGlobal);
      if (verifyInstalled(buildmateDest, 'commands/buildmate')) {
        const cmdCount = fs.readdirSync(buildmateDest).filter(f => f.endsWith('.md')).length;
        console.log(`  ${green}✓${reset} Installed ${cmdCount} Buildmate V2 commands`);
      } else {
        failures.push('commands/buildmate');
      }
    }
  }

  // V2: Skip buildmate-core legacy installation - GSD templates removed
  // New V2 agents are in /agents/ directory, commands in /commands/
  // Still create buildmate-core/ for VERSION and CHANGELOG.md
  const buildmateCoreDir = path.join(targetDir, 'buildmate-core');
  if (!fs.existsSync(buildmateCoreDir)) {
    fs.mkdirSync(buildmateCoreDir, { recursive: true });
  }
  console.log(`  ${green}✓${reset} Created buildmate-core/`);
  ...
  const changelogDest = path.join(targetDir, 'buildmate-core', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'buildmate-core', 'VERSION');
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'buildmate-core', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true) {
  ...
  ${cyan}View source:${reset} https://github.com/sprajapati024/buildmate
`);
}
